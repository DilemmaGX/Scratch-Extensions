...
class CoLang {
    ...
    argsLine(args) {
        let str = args.line;
        // 根据args.type确定括号类型  
        const openBracket = args.type === 0 ? '(' : args.type === 1 ? '[' : '{';
        const closeBracket = args.type === 0 ? ')' : args.type === 1 ? ']' : '}';

        // 根据括号类型匹配值  
        const match = str.match(new RegExp(`\\${openBracket}(.*?)${closeBracket}`, 'g'));
        if (match) {
            const values = match.map(value => value.slice(openBracket.length, -closeBracket.length));
            return values;
        } else {
            return [];
        }
